...
use specs::{Entities, ReadStorage, WriteStorage, Join, Write, ReadExpect, Read};

use crate::server::protocol::Score;
use crate::server::protocol::server::ScoreUpdate;
use crate::server::component::counter::{Earnings, TotalKills, TotalDeaths, PlayersGame};
use crate::server::component::flag::IsPlayer;
use crate::server::types::{Team, Upgrades, Config, Health};
...
pub async fn new_game(mut data: TaskData, winner: Team) {
	...
	display_message(&mut data, "New game starting in 1 minute", Duration::from_secs(12));

	data.sleep_for(Duration::from_secs(30)).await;

	shuffle(&mut data);
	display_message(&mut data, "Game starting in 30 seconds", Duration::from_secs(7));

	data.sleep_for(Duration::from_secs(20)).await;
	display_message(&mut data, "Game starting in 10 seconds", Duration::from_secs(7));

	data.sleep_for(Duration::from_secs(5)).await;
	display_message(&mut data, "Game starting in 5 seconds", Duration::from_secs(2));

	data.sleep_for(Duration::from_secs(1)).await;
	display_message(&mut data, "Game starting in 4 seconds", Duration::from_secs(2));

	data.sleep_for(Duration::from_secs(1)).await;
	display_message(&mut data, "Game starting in 3 seconds", Duration::from_secs(2));

	data.sleep_for(Duration::from_secs(1)).await;
	display_message(&mut data, "Game starting in 2 seconds", Duration::from_secs(2));

	data.sleep_for(Duration::from_secs(1)).await;
	display_message(&mut data, "Game starting in a second", Duration::from_secs(2));
...
fn award_bounty(data: &mut TaskData, winner: Team) {
	...
	data.world(|world| {
		...
		(
			...
			.for_each(|(_, score, earnings, player, kills, deaths, upgrades, ..)| {
				score.0 += bounty;
				(earnings.0).0 += bounty;
...
fn reset_flags(data: &mut TaskData) {
	use crate::component::{OnFlag, FlagEvent, Flags, FlagEventType};
...
fn shuffle(data: &mut TaskData) {
	use crate::shuffle::{ShuffleProvider, PlayerShuffleInfo};
	...
	use crate::config::{RED_TEAM, BLUE_TEAM};
	...
	data.world(|world| {
		...
		let player_info = (
			...
			.join()
			.map(|(id, team, score, captures, kills, deaths, ..)| {
				PlayerShuffleInfo {
					player: id,
					team: *team,
					score: *score,
...
fn respawn_all_players(data: &mut TaskData) {
	...
	use crate::server::component::flag::{IsSpectating, IsDead};
...
fn reset_scores(data: &mut TaskData) {
	use crate::component::{Flags, GameScores, OnFlag, FlagEvent, FlagEventType};
