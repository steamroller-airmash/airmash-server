...
impl<'a> System<'a> for ScoreBoardTimerHandler {
	...
	fn setup(&mut self, res: &mut Resources) {
		self.reader = Some(
			res.fetch_mut::<EventChannel<ScoreBoardTimerEvent>>().register_reader()
		);
	...
	fn run(&mut self, data: Self::SystemData) {
		if let Some(ref mut reader) = self.reader {
			for _ in data.channel.read(reader) {
				let mut packet_data = 
					(&*data.entities, &data.scores, &data.levels).join()
				...
				let packet_data = packet_data.into_iter()
					.take(10)
					.collect::<Vec<ScoreBoardData>>();

				let rankings = (&*data.entities, &data.pos).join()
