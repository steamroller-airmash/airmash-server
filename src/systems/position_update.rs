...
impl PositionUpdate {
	...
	fn step_players<'a>(
		...
		.for_each(|(pos, rot, speed, keystate, upgrades, powerups, plane)| {
			...
			let boost_factor = if Self::boost(&plane, keystate) { info.boost_factor } else { 1.0 };
			...
			*rot = (*rot % PIx2 + PIx2) % PIx2;
			...
			let bound = Position::new(
				Distance::new(16352.0),
				Distance::new(8160.0)
			);

			if pos.x.abs() > bound.x { pos.x = pos.x.signum() * bound.x }
			if pos.y.abs() > bound.y { pos.y = pos.y.signum() * bound.y }
		});
	}
	...
	fn send_updates<'a>(
		...
		.for_each(|(pos, rot, speed, plane, keystate, upgrades, powerups, ent, _)| {
...
impl<'a> System<'a> for PositionUpdate {
	...
	fn run(&mut self, (mut data, config): Self::SystemData) {
		self.dirty.clear();
		data.keystate.populate_modified(
			&mut self.modify_reader.as_mut().unwrap(),
			&mut self.dirty
		);
