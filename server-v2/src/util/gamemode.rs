//! Note: This entire module is an exercise in bashing the trait system over
//! the head until it lets us do what we want safely.

use crate::ecs::prelude::*;
use crate::ecs::{SystemData, World};
use crate::protocol::{Team, PlaneType as Plane, Position};
	...
	T: GameMode + Sync + Send + ?Sized,
{
	...
	fn reads(types: &mut Vec<TypeId>) {
		<WriteExpect<'a, GameModeInternal> as SystemData>::reads(types)
	}

	fn writes(types: &mut Vec<TypeId>) {
		<WriteExpect<'a, GameModeInternal> as SystemData>::writes(types)
	}
}
