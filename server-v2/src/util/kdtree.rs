...
impl<T> KdTree<T> {
    ...
    pub fn contains_any(&self, point: Vector2<f32>, radius: f32) -> bool {
        self.lookup_rough(point, radius).next().is_some()
    }

    fn lookup_rough<'a>(&'a self, point: Vector2<f32>, radius: f32) -> RoughLookupIterator<'a, T> {
        RoughLookupIterator::new(self, point, radius)
    }

    pub fn lookup<'a>(&'a self, point: Vector2<f32>, radius: f32) -> impl Iterator<Item = &'a T> {
        self.lookup_rough(point, radius)
            .filter(move |x| {
...
struct RoughLookupIterator<'a, T> {
    tree: &'a KdTree<T>,
    query: AABB,
    state: Vec<State>,
}

impl<'a, T> RoughLookupIterator<'a, T> {
    pub fn new(tree: &'a KdTree<T>, point: Vector2<f32>, radius: f32) -> Self {
        let query_rad = radius + tree.max_radius;
        let mut query = AABB {
            x: (point.x - query_rad)..=(point.x + query_rad),
            y: (point.y - query_rad)..=(point.y + query_rad),
        };
        query.clip(&tree.bounds);
        ...
        if tree.is_empty() {
            ...
            Self {
                tree,
                query,
                state: vec![State { index: 0, level: 0 }],
            }
        }
    }
}

impl<'a, T> Iterator for RoughLookupIterator<'a, T> {
    type Item = LookupResult<'a, T>;

    fn next(&mut self) -> Option<Self::Item> {
        loop {
            let State { index, level } = self.state.pop()?;
            ...
            match node {
                &KdTreeNode::Children { split, right, left } => {
                    ...
                    assert!(level < self.tree.nodes.len());
...
mod tests {
    ...
    fn points_in_same_location() {
        ...
        assert!(tree.lookup(Vector2::new(1.0, 1.0), 0.5).count() == 2);
    }
}
