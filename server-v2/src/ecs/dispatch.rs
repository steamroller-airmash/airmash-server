...
impl<'world> Builder<'world> {
	...
	pub fn with<S: SystemBuilder + 'static>(&mut self, system: S) -> &mut Self {
		self.builders.push(Box::new(DynSystemBuilderImpl::new(system)));
		self
	}

	pub fn build(mut self) -> Result<Dispatcher, CycleError> {
		use petgraph::{Graph, Directed, algo::toposort};
...
impl fmt::Display for CycleError {
	fn fmt(&self, fmt: &mut fmt::Formatter) -> fmt::Result {
		writeln!(fmt, "Encountered a cycle containing the following system: {}", self.system)
	}
}
