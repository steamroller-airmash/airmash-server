...
impl<M> AnyMap<M> {
    ...
    pub fn insert<T: 'static>(&mut self, value: T, meta: M) -> &mut T {
        self.map.entry(TypeId::of::<T>()).insert(MapItem {
            meta,
            value: Box::new(value),
        });
...
mod test {
    ...
    fn insert_get() {
        ...
        assert_eq!(map.get_mut::<A>().map(|(&mut x, _)| x), Some(A));
        assert_eq!(map.get_mut::<B>().map(|(&mut x, _)| x), None);

        assert_eq!(map.get::<A>().map(|(&x, _)| x), Some(A));
        assert_eq!(map.get::<B>().map(|(&x, _)| x), None);
    }
    ...
    fn insert_iter_mut() {
        ...
        let elemptrs: Vec<_> = map
            .iter_mut()
            .map(|x| x.1 as *const _ as *const ())
            .collect();
