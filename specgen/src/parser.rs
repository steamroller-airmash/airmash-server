    ...
    fn tokenize(b: &'a [u8]) -> Result<Vec<Token<'a>>, Error<'a>> {
        ...
        while i < b.len() {
            ...
            let data = match b[i] as char {
                ...
                '{' => Token {
                    line,
                    ty: TokenData::LBracket,
                },
                '}' => Token {
                    line,
                    ty: TokenData::RBracket,
                },
                '[' => Token {
                    line,
                    ty: TokenData::LBrace,
                ...
                'a'...'z' | 'A'...'Z' => {
                    ...
                    loop {
                        ...
                        if b[i + len] > 127 {
                            return Err(Error::new(ErrorType::NonAsciiCodepoint(b[i + len]), line));
                        }
                    ...
                    let s = match str::from_utf8(&b[i..i + len]) {
                        Ok(s) => s,
                        Err(e) => return Err(Error::new(ErrorType::Utf8Error(e), line)),
                    };

                    i += len - 1;

                    Token {
                        ty: TokenData::Ident(s),
                        line,
                    }
                }
                '#' => {
                ...
                '0'...'9' => {
                    ...
                    loop {
                        ...
                        match b[i + len] as char {
                            ...
                            'A'...'Z' | 'a'...'z' | '_' => {
                                ...
                                return Err(Error::new(ErrorType::InvalidIntegerLiteral(s), line));
                            }
                            _ => break,
                        }
                    }
                    ...
                    let num = match s.parse() {
                        Ok(n) => n,
                        Err(_) => return Err(Error::new(ErrorType::InvalidIntegerLiteral(s), line)),
                    };

                    i += len - 1;

                    Token {
                        ty: TokenData::Integer(num),
                        line,
                    }
                }
...
fn parse_kw<'a>(tokens: &mut Tokens<'a>, kw: &'static str) -> Result<(), Error<'a>> {
    ...
    Err(Error::new(ErrorType::ExpectingKeyword(kw, ty), line))
}
...
fn parse_lbracket<'a>(tokens: &mut Tokens<'a>) -> Result<(), Error<'a>> {
    ...
    if ty != TokenData::LBracket {
        return Err(Error::new(ErrorType::ExpectingRBracket(ty), line));
    }
...
fn parse_comma<'a>(tokens: &mut Tokens<'a>) -> Result<(), Error<'a>> {
    ...
    if ty != TokenData::Comma {
        return Err(Error::new(ErrorType::ExpectingComma(ty), line));
    }
...
fn parse_colon<'a>(tokens: &mut Tokens<'a>) -> Result<(), Error<'a>> {
    ...
    if ty != TokenData::Colon {
        return Err(Error::new(ErrorType::ExpectingColon(ty), line));
    }
...
fn parse_equals<'a>(tokens: &mut Tokens<'a>) -> Result<(), Error<'a>> {
    ...
    if ty != TokenData::Equals {
        return Err(Error::new(ErrorType::ExpectingEquals(ty), line));
    }
