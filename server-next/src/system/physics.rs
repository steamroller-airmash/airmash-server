...
fn update_player_positions(game: &mut AirmashWorld) {
  ...
  for (_, (pos, rot, vel, keystate, upgrades, powerup, plane)) in query {
    ...
    let bound = Vector2::new(16352.0, 1860.0);
...
fn send_update_packets(game: &mut AirmashWorld) {
  ...
  for (ent, (pos, rot, vel, plane, keystate, upgrades, powerup, last_update, team)) in query.iter()
  {
    if last_update.0.elapsed() < Duration::from_secs(1) {
      continue;
    }
