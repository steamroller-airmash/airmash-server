...
impl<'de> Deserialize<'de> for ServerPacket {
	fn deserialize(de: &mut Deserializer<'de>) -> Result<ServerPacket, DeError> {
		...
		Ok(match de.deserialize_u8()? {
			...
			EVENT_LEAVEHORIZON => {
				ServerPacket::EventLeaveHorizon(EventLeaveHorizon::deserialize(de)?)
			}
			MOB_UPDATE => ServerPacket::MobUpdate(MobUpdate::deserialize(de)?),
			MOB_UPDATE_STATIONARY => {
				ServerPacket::MobUpdateStationary(MobUpdateStationary::deserialize(de)?)
			}
			MOB_DESPAWN => ServerPacket::MobDespawn(MobDespawn::deserialize(de)?),
			MOB_DESPAWN_COORDS => {
				ServerPacket::MobDespawnCoords(MobDespawnCoords::deserialize(de)?)
			}
			...
			CHAT_VOTEMUTEPASSED => {
				ServerPacket::ChatVoteMutePassed(ChatVoteMutePassed::deserialize(de)?)
			}
			...
			SCORE_DETAILED_FFA => {
				ServerPacket::ScoreDetailedFFA(ScoreDetailedFFA::deserialize(de)?)
			}
			SCORE_DETAILED_CTF => {
				ServerPacket::ScoreDetailedCTF(ScoreDetailedCTF::deserialize(de)?)
			}
			SCORE_DETAILED_BTR => {
				ServerPacket::ScoreDetailedBTR(ScoreDetailedBTR::deserialize(de)?)
			}
