...
use crate::component::{
	channel::{
		OnEnterHorizon, OnLeaveHorizon, OnMissileDespawn, OnMissileFire, OnPlayerLeave,
		OnPlayerRespawn, OnPowerupDespawn, OnPowerupSpawn,
	},
	collision::{MissileGrid, PlayerGrid, PowerupGrid},
	event::{EnterHorizon, EntityType, LeaveHorizon, MissileDespawnType},
...
struct Channels<'a> {
	...
	missile_fire: Read<'a, OnMissileFire>,
...
pub async fn calculate_visibility(mut data: TaskData) {
	...
	let mut missile_fire_reader =
		data.write_resource::<OnMissileFire, _, _>(|mut res| res.register_reader());
	let mut powerup_spawn_reader =
		data.write_resource::<OnPowerupSpawn, _, _>(|mut res| res.register_reader());
	let mut player_respawn_reader =
		data.write_resource::<OnPlayerRespawn, _, _>(|mut res| res.register_reader());
	...
	loop {
		...
		let newly_appeared = data.world(|world| {
			...
			let players = channels
				.player_respawn
				.read(&mut player_respawn_reader)
				.map(|evt| evt.player);
			let missiles = channels
				.missile_fire
				.read(&mut missile_fire_reader)
				.flat_map(|evt| evt.missiles.iter().copied());
			let powerups = channels
				.powerup_spawn
				.read(&mut powerup_spawn_reader)
				.map(|evt| evt.mob);
		...
		data.world(|world| {
			...
			for (player, new_visible) in &visible {
				...
				for &ent in new_visible {
					if !old_visible.contains(&ent) {
						let ty = match () {
							// Ignore dead entities
							_ if !data.entities.is_alive(ent) => continue,
							_ if data.is_player.get(ent).is_some() => EntityType::Player,
							_ if data.is_missile.get(ent).is_some() => EntityType::Missile,
							_ if data.is_powerup.get(ent).is_some() => EntityType::Powerup,
							_ => {
						...
						data.enter_horizon.single_write(EnterHorizon {
							player: *player,
							entered: ent,
							entered_ty: ty,
							just_spawned: newly_appeared.contains(&ent),
						});
					}
				}
			}
