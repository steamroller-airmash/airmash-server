  ...
  fn send_events<'a>(&mut self, data: &mut TrackVisibleData<'a>) {
    ...
    (&*data.entities, &data.pos, data.is_player.mask())
      ...
      .for_each(|(ent, pos, ..)| {
        ...
        let players = Self::rough_collide(*pos, ent, &players.0, &*config, EntityType::Player);
        let missiles = Self::rough_collide(*pos, ent, &missiles.0, &*config, EntityType::Missile);
        let powerups = Self::rough_collide(*pos, ent, &powerups.0, &*config, EntityType::Powerup);
